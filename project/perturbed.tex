...
%TODO: Add equations for 2,5 and 3,6
...
impose
%TODO: Fix blabber on periodic boundary condition
...
irreducibled Brillouin zone for our ribbon system is.
...
\caption{\label{fig:hexstripM} The effect of alternating masses (with the top
  and bottom layer the same configuration) on the dispersion relation of the
  hexagonal strip system, with all other parameters set to $1$.}
\end{figure}
...
essentially
%TODO: What is the significance of two different materials?
...
\caption{\label{fig:hexstripMrotated} The effect of alternating masses (with
  bottom layer as a $\frac{2\pi}{6}$ rotation of the top layer) on the
  dispersion relation of the hexagonal strip system, with all other parameters
  set to $1$.}
\end{figure}
...
With this knowledge, we are then able to start trying to force energy to
propagate in the directions we desire in Chapter \ref{scattering}.
...
Figure~\ref{fig:hexperturb2}, we have one increased and one decreased system
which have similar bandgaps. As such, we will be using those two configurations
to form our strip.

\begin{figure}[!h]
\centering
\includegraphics[width=0.8\textwidth]{imgs/hexstripperturb2.png}
\caption{\label{fig:hexstrip2} The effect of having the top material of a
  hexagonal strip have higher $M_i$ and $k$ than the bottom material on the
  dispersion relation of the bulk kagome system, with all other parameters set
  to $1$.}
\end{figure}

As before we again get a well-defined bandgap, as well as two wave states which
are present in the bandgap and therefore can propagate along the interface.
